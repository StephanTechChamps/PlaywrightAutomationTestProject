...
const ProductItem: React.FC<ProductItemProps> = ({ product, cartItem, currency, addToCart, removeFromCart }) => (
  <li style={{ display: 'flex', alignItems: 'center', marginBottom: 24, gap: 16, padding: '18px 0', borderBottom: '1px solid #eee', minHeight: 90 }}>
    {product.image && (
      <img
        src={product.image}
        alt={product.name}
    ...
    <div style={{ flex: 1, display: 'flex', alignItems: 'center' }}>
      <span style={{ fontWeight: 500, fontSize: 16, flex: 1 }}>{product.name}</span>
      <span style={{ color: '#888', fontSize: 15, minWidth: 60, textAlign: 'right' }}>{formatPrice(getUnitPrice(product, currency), currency)}</span>
    </div>
    <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', marginTop: 16 }}>
      <button style={{ minWidth: 110 }} onClick={() => addToCart(product)}>
        Add to Cart
      </button>
